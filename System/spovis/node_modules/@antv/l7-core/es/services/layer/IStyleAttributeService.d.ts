...
export declare type ScaleTypeName = 'linear' | 'power' | 'log' | 'identity' | 'time' | 'quantile' | 'quantize' | 'threshold' | 'cat';
...
export declare enum StyleScaleType {
    CONSTANT = "constant",
    VARIABLE = "variable"
}
...
export interface IVertexAttributeDescriptor extends Omit<IAttributeInitializationOptions, 'buffer'> {
    ...
    update?: (feature: IEncodeFeature, featureIdx: number, vertex: number[], attributeIdx: number, normal: number[]) => number[];
}
declare type Position = number[];
declare type Color = [number, number, number, number];
declare type CallBack = (...args: any[]) => any;
export declare type StyleAttributeField = string | string[] | number[];
...
export declare type Triangulation = (feature: IEncodeFeature) => {
    vertices: number[];
    indices: number[];
    size: number;
    normals?: number[];
...
export interface IStyleAttributeService {
    ...
    updateStyleAttribute(attributeName: string, attributeOptions: Partial<IStyleAttributeInitializationOptions>, updateOptions: IStyleAttributeUpdateOptions): void;
    getLayerStyleAttributes(): IStyleAttribute[] | undefined;
    getLayerStyleAttribute(attributeName: string): IStyleAttribute | undefined;
    createAttributesAndIndices(encodedFeatures: IEncodeFeature[], triangulation?: Triangulation): {
        attributes: {
            [attributeName: string]: IAttribute;
        };
        elements: IElements;
